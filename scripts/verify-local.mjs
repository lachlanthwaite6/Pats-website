...
const read = () => call("/api/v1/dashboard");
const start = await read();
assert.equal(start.status, 200);
const original = await start.json();
const firstId = start.headers.get("ETag").replaceAll('"', "");
...
assert.equal(
  (await call("/api/v1/dashboard", { method: "DELETE" })).status,
  405,
);
assert.equal((await call("/private/FLIPPEDupdated.xlsx")).status, 404);
assert.equal(
  (
    await call("/api/v1/dashboard", {
      headers: { "If-None-Match": check.headers.get("ETag") },
    })
  ).status,
  304,
);
console.log(
  "Local integration checks passed: valid/idempotent import, invalid import rejection, unauthorised writes, rollback, missing version, private-file exclusion and ETag.",
);
