...
| Route                     | Access                          | Result                                                      |
| ------------------------- | ------------------------------- | ----------------------------------------------------------- |
| GET /api/health           | Public                          | Liveness and release version; not a D1 readiness check      |
| GET /api/v1/dashboard     | Public in current configuration | Full validated aggregate snapshot, 60-second cache and ETag |
...
Set DATA_ACCESS=access, ACCESS_TEAM_DOMAIN=https://your-team.cloudflareaccess.com and ACCESS_AUD to the Access application's audience, then configure Cloudflare Access policy for the API hostname/path. JWT verification checks signature, issuer, audience and expiry; missing configuration denies reads. Private data never enters the shared API cache. The generic static shell remains public. Previously published public data cannot be made secret retroactively. Private mode is implemented but not enabled or account-integration-tested in this public deployment.
